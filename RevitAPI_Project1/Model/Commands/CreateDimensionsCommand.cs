...
namespace RevitAPI_Project1
{
    ...
    public class CreateDimensionsCommand : IExternalCommand
    {
        ...
        public Result Execute(ExternalCommandData commandData, ref string message, ElementSet elements)
        {
            ...
            try
            {
                using (Transaction transactionn = new Transaction(Doc, "Create Dim"))
                {
                    transactionn.Start();

                    CreateDimensions(Doc, GridCreator.HorizontalGrids, isHorizontal: true, distanceBetweenHorizontalGrid);
                    CreateDimensions(Doc, GridCreator.VerticalGrids, isHorizontal: false, distanceBetweenVerticalGrid);
